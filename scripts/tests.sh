...
function acceptanceTests() {
  ...
  java -jar -Dspring.profiles.active=test ${BASE_DIR}/target/blackpearltwo-[0-9\.]*-SNAPSHOT.jar --server.port=9090 &>${BASE_DIR}/tmp/acceptance.log &
  echo $! >${BASE_DIR}/tmp/blackpearltwo.pid

  testConnection ${REACT_APP_HOST} $(cat ${BASE_DIR}/tmp/blackpearltwo.pid)

  pushd ${BASE_DIR}/acceptance
  yarn install
          if [[ "${BLACKPEARLTWO_CI}" && "$(lsb_release -crid | grep -i 'Ubuntu')" ]]; then
...
function cleanup() {
  showBanner "Cleanup"
  if [[ -f ${BASE_DIR}/tmp/blackpearltwo.pid ]]; then
    cat ${BASE_DIR}/tmp/blackpearltwo.pid | xargs kill -9
    rm ${BASE_DIR}/tmp/blackpearltwo.pid
  fi
}
trap cleanup EXIT
...
function jarBuild() {
  ...
  pushd ${BASE_DIR}
  mvn -Dmaven.test.skip=true -DskipTests -Dflyway.user=${BLACKPEARLTWO_DB_USERNAME} -Dflyway.password= -Dflyway.url=${BLACKPEARLTWO_DB_URL} clean flyway:migrate package
